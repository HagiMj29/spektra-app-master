...
class AchievementController extends Controller
{
    ...
    public function datatable(){
        $data = DB::table('t_achievement')->get();
    ...
    public function create()
    {
        ...
        ->join('t_student', 't_achievement.student_id', '=', 't_student.student_id')
        ->select('t_achievement.*')
        ->first();

    $student = DB::table('t_student')->get();

    return view('achievement.create', ['data' => $data, 'students'=>$student ]);
    }
    ...
    {
        $data = DB::table('t_achievement')
        ->join('t_student', 't_achivement.student_id', '=', 't_student.student_id')
        ->select('t_achievement.*')
        ...
        if(!$validator->passes()){
            ...
            $data->update($request->all());
             if( $data->update($request->all()) ){
               return response()->json(['status'=>1, 'url'=>'/achievement','message'=>'Achievement Updated Succesfully!']);
           }
        }
    }
